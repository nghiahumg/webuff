<!--ĐƠN VỊ THIẾT KẾ WEBSITE NTIENDAT | ZALO: 0395236709 | TELE: @nopro81 | KHÔNG SỬ DỤNG MÃ NGUỒN CỦA BÊN KHÁC CUNG CẤP CHÚNG TÔI KHÔNG BẢO HÀNH LỖI
-->
<?php
...
$so_tien=$tien_chuyen+$tien_chuyen*$NTD->site('nap_km_bank')/100;
...
if ($ma_baoMat == $api_momo) {
      $createtien = $NTD->insert("history_naptien", [
                            'date' => $thoi_gian,
                            'type' => 'Bank',
                            'loaithe' => $ten_bank,
                            'username'      => $id_khach,
      ...
      if($createtien){
        ...
        $update1= $NTD->cong("users", "money", $so_tien, " `username` = '$id_khach' AND  `url_config` = '".$url_site_name."'");
        $update3= $NTD->cong("users", "tongnap", $so_tien, " `username` = '$id_khach' AND  `url_config` = '".$url_site_name."'");
         $return['status'] = false;
        $return['error'] = true;
