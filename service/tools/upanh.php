...
// $selectservice = $NHQ->get_row("SELECT * FROM `rate_service` WHERE `code_service` = 'sub_shopee' AND `status_service` = '1' "); 
$getrate = $NHQ->get_list("SELECT * FROM `server_service` WHERE `code_service` = 'sub_shopee' AND `status_service` = '1' ");
}?>
...
<div class="card p-3">
<div class="d-flex align-items-center">
<div class="image"> <img src="<?=$NHQ->site('logo_user');?><?=$NHQ->getUsers('username');?>" class="rounded" width="155"> </div>
<div class="ml-3 w-100">
<h4><a><?=$NHQ->getUsers('username');?></a></h4>
<span><?=levelmem($NHQ->getUsers('capbac'));?></span>
<div class="p-2 mt-2 bg-primary d-flex justify-content-between rounded text-white stats">
<div class="d-flex flex-column"> <span class="statistical">Số dư</span> <span class="number"><?=format_money($NHQ->getUsers('money'));?> coin</span> </div>
<div class="d-flex flex-column"> <span class="statistical">Đã nạp</span> <span class="number"><?=format_money($NHQ->getUsers('tongnap'));?> coin</span> </div>
<div class="d-flex flex-column"> <span class="statistical">Đã tiêu</span> <span class="number"><?=format_money($NHQ->getUsers('tongtru'));?> coin</span> </div>
</div>
