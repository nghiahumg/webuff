...
<script type="b9e3e84309a92aaf852234bf-text/javascript">
    const profile = {
        ...
        "website": [<?=$NTD->getUsers('domain_sitecon');?>],
...
<div class="page-title">Lấy ID facebook</div>
...
<div class="card">
<div class=" card-body">
 <form submit-ajax="NTD" method="post" action="<?=BASE_URL('api/IDFB');?>">
                            <input type="hidden" name="_token" value="9jILCFLdUiKHj5NIvPwKO98dWO5obzeL3msiDKcQ">
<div class="row">

<div class="form-group col-md-12 mb-3">
<o id="callback"></o>
                                            <input type="url" class="form-control mb-3" name="Link" placeholder="https://www.facebook.com/">


</div>
<center><button type="submit" callback="rescallback" class="btn btn-primary"> Tìm Kiếm</button></center></div>
</form>
</div></div>
</div>
</div>
...
<script>
    function rescallback(response){if (response.status === true){$('.rescallback').show();$('#callback').html(response.data);}else{$('.rescallback').hide();$('#callback').html(response.data);}}
    </script>
