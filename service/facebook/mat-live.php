<?php require('../../config/function.php'); $dtb="mat_live"; $action="mat-live"; $title="Mắt Live Facebook";?><?php require('../../config/head.php'); if(empty($_SESSION['username'])){header('Location: /Login');exit;}else{$getrate=$NTD->get_list("SELECT * FROM `server_service` WHERE `code_service`='$dtb' AND `status_service`='1' AND `url_config`='".$url_site_name."' ");}?>
...
<label for="" class="form-label col-md-3">ID bài viết </label>
<div class="col-md-9">
<input class="form-control" name="id"  placeholder="Nhập id hoặc nhập link bài viết hệ thống tự get id" onchange="getUID('id');" onchange="bill();">
...
<div class="col-md-9">
<input type="text" class="form-control mb-3" name="time_buy" onkeyup="bill();" value="50" placeholder="Nhập số phút cần tăng">
...
- <b>Mua bằng ID bài viết ở chế độ công khai</b>.<br>
- <b>Không hỗ trợ bài viết trong nhóm, không chạy livestream chia sẻ âm nhạc</b>.
</div> </div>
</div>
<div class="form-group row mb-3">
...
<script>
        ...
        <?php }?>
        let total_payment = Math.round(soluong * price2 * time);
