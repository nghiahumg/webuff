<?php require('../../config/function.php'); $dtb="mem_gr"; $action="mem-group"; $title="Thành Viên Nhóm";?><?php require('../../config/head.php'); if(empty($_SESSION['username'])){header('Location: /Login');exit;}else{$getrate=$NTD->get_list("SELECT * FROM `server_service` WHERE `code_service`='$dtb' AND `status_service`='1' AND `url_config`='".$url_site_name."' ");}?>
...
<div class="col-md-9">
<input class="form-control" name="id"  placeholder="Nhập id group hoặc link hệ thống tự get" onchange="getUID('id');" onpaste="bill();">
...
- <b>Mua bằng ID Group phải mở chế độ công khai, tắt phê đuyệt để member có thể tham gia
được luôn.</b>.
</div></div>
</div>
