...
$dtb="sub_sale"; $action="sub-sale"; $title="Follow Giảm Giá";?><?php require('../../config/head.php'); if(empty($_SESSION['username'])){header('Location: /Login');exit;}else{$getrate=$NTD->get_list("SELECT * FROM `server_service` WHERE `code_service`='$dtb' AND `status_service`='1' AND `url_config`='".$url_site_name."' ");}?>
...
<div class="col-md-9">
<input class="form-control" name="id"  placeholder="Nhập id hoặc nhập link profile" onchange="getUID('id');" onchange="bill();">
...
- <b>Mua bằng ID Facebook đã mở chế độ công khai, có nút theo dõi, mở nút kết bạn,
khi
có yêu cầu kết bạn không được đồng ý để k tụt sub.</b>.
</div> </div>
</div>
<div class="form-group row mb-3">
