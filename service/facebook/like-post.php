...
$text=getCurURL();$dichvu1=(explode('/', $text));$dich_vu=$dichvu1['5'];
if($dich_vu=="like-post"){
  
$dtb="like_post"; $action="like-post"; $title="Tăng Like Bài Viết";
}
if($dich_vu=="like-post-sale"){
$dtb="like_post_sale"; $action="like-post-sale"; $title="Tăng Like Bài Viết Sale";
}
?>

<?php require('../../config/head.php'); 

if(empty($_SESSION['username'])){header('Location: /Login');exit;}else{$getrate=$NTD->get_list("SELECT * FROM `server_service` WHERE `code_service`='$dtb' AND `status_service`='1' AND `url_config`='".$url_site_name."' ");}?>
...
<?php if($action=="like-post"){?><label for="" class="form-label col-md-3">ID bài viết </label><?php }else{?><label for="" class="form-label col-md-3">Link bài viết </label><?php }?>
<div class="col-md-9">
<input class="form-control" name="id"  <?php if($action=="like-post"){?>placeholder="Nhập id bài viết hoặc nhập link" onpaste="getUID('id');"<?php }else{?> type="url" placeholder="Nhập link link cần tăng"<?php }?> onchange="bill();">
<?php if($action=="like-post"){?><small class="form-text text-muted">Lưu ý: Riêng đối với buff like cho Avatar và ảnh
Bìa hãy ấn thẳng vào ảnh rồi copy link, hoặc có thể xem hướng dẫn: <a href="https://i.imgur.com/QE8JFYT.png" target="_blank" class="font-bold">
Tại đây</a></small><?php }?>
</div>
</div>
<div class="form-group row mb-3">
...
- <b>Mua bằng link bài viết ở chế độ công khai, phải có nút like.</b>.
</div> </div>
</div>
<div class="form-group row mb-3">
