<?php require('../../config/function.php');$text=getCurURL();$dichvu1=(explode('/', $text));$dich_vu=$dichvu1['5'];if($dich_vu=="cmt"){$dtb="cmt_ins"; $action=$dichvu1['5']; $title="Tăng Comment Bài Viết";}elseif($dich_vu=="like"){$dtb="like_ins"; $action=$dichvu1['5']; $title="Like Bài Viết";}elseif($dich_vu=="follow"){$dtb="sub_ins"; $action=$dichvu1['5']; $title="Follow Instagram";}?><?php require('../../config/head.php'); if(empty($_SESSION['username'])){header('Location: /Login');exit;}else{$getrate=$NTD->get_list("SELECT * FROM `server_service` WHERE `code_service`='$dtb' AND `status_service`='1' AND `url_config`='".$url_site_name."' ");}?>
<title><?=$title;?></title>
...
<div class="col-md-8 mb-3">
   <form submit-ajax="NTD" action="<?=BASE_URL('api/instagram/buy');?>" method="post" api_token="<?=$NTD->getUsers('token');?>">
                                     <input type="hidden" name="action" value="<?=$action;?>">
                                    <input type="hidden" name="code_dich_vu" value="<?=$dtb;?>">
<div class="form-group row mb-3">
<label for="" class="form-label col-md-3">Link video </label>
<div class="col-md-9">
<input <?php if($action=="follow"){?> type="text" placeholder="Nhập username cần tăng" <?php }else{?>type="url" placeholder="Nhập link video cần tăng"<?php }?> class="form-control" name="id"   onchange="bill();">
</div>
</div>
<div class="form-group row mb-3">
...
<div class="form-group row mb-3">
    ...
    <div class="col-md-9">
        <textarea class="form-control" name="list_msg" rows="3" placeholder="Nhập nội dung bình luận, mỗi nội dung 1 dòng"></textarea>
    </div>
</div>
<?php }?>
<div class="form-group row mb-3">
...
<div class="alert bg-danger text-white" role="alert">
    ...
    - <b>Mua bằng link video có dạng https://www.tiktok.com/@username/video/id_video</b>.
    <br />
    - <b>Tài khoản cần phải tắt chế độ riêng tư, nghiêm cấm đổi Username trong quá trình tăng, cố tình đổi Username không hỗ trợ</b>.
</div>
 </div>
</div>
<div class="form-group row mb-3">
