...
<div class="row">
    <div class="col-md-12">
        <div class="card mb-3">
            ...
            <div class="card-body">
                <form submit-ajax="NTD" action="<?= BASE_URL('api/Admin-Momo-Ajax'); ?>" method="post" api_token="<?= $NTD->getUsers('token'); ?>">
                        ...
                        <table class="table table-bordered table-striped fs--1 mb-3 dataTables-NTD dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                            ...
                            <tbody>
                                ...
                                if ($url_site_name == $URL_ADMIN) {
                                    $getlistcard = $NTD->get_list("SELECT * FROM `cron_momo`  ORDER BY `id` DESC ");
                                } else {
                                    $getlistcard = $NTD->get_list("SELECT * FROM `cron_momo` WHERE  `sitename` = '" . $url_site_name . "' ORDER BY `id` DESC ");
                                }
                                if ($getlistcard) {
