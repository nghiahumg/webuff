...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
  ...
  if($action=="them_api"){
    ...
    if(empty($ten_api)){
     ...
     if(empty($token_api)){
        ...
        die(json_encode($return));
          ...
          $api_token = api_token();
    $user_api = $NTD->get_row(" SELECT * FROM `users` WHERE `token` = '$api_token'  AND  `url_config` = '".$url_site_name."' ");
    ...
    if($token_api!==$user_api['token']){
     ...
     if($NTD->num_rows(" SELECT * FROM `list_api` WHERE `username` = '".$user_api['username']."' ") >= 3)
                                {
                                   $return['status'] = false;
        $return['error'] = true;
   ...
   curl_close($ch); 
    ...
    $action2= check_string($_POST['action2']);
       ...
       $token = check_string($_POST['token']);
         $check_name = $NTD->get_row(" SELECT * FROM `users` WHERE `token` = '$token'  AND  `url_config` = '".$url_site_name."' ");
        
        if($action2=="duyet"){
            
        
          $update = $NTD->update("list_api", [
        'status' => 'active'
