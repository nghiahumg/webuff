...
if($url_site_name==$domain_site_me){
   ...
   $check_ds_site = $NTD->get_row("SELECT * FROM `ds_sitecon` WHERE `site_con` = '$domain_site_con' AND `site_me` = '$domain_site_me' AND `status` = 'Active'");
   
   
   
 $username_site_con = $NTD->get_row(" SELECT * FROM `users` WHERE `token` = '".$check_ds_site['token']."'  AND  `url_config` = '".$domain_site_me."'
");
 if($action=="sub-sale"){
...
$check_site_me_config = $NTD->get_row(" SELECT * FROM `setting` WHERE `url_config` = '".$domain_site_me."'   ");
...
if($action=="vip-like"){
  ...
  $get_site_me_3 = $NTD->get_row(" SELECT * FROM `ds_sitecon` WHERE `site_con` = '".$domain_site_me."'");

    if($get_site_me_3){
      $dm_site_me=$get_site_me_3['site_me'];
       $dm_site_con=$domain_site_me;
$mien_site_me_thuong=strtolower($dm_site_me);
$url_get="https://$mien_site_me_thuong/auth/service/facebook/site_con.php?id=$id&soluong=$soluong&server=$server_buy&code_dich_vu=$code_dich_vu&action=$action&loai_camxuc=$loai_camxuc&time_buy=$time_buy&list_msg=$list_msg&domain_site_con=$dm_site_con&domain_site_me=$dm_site_me&so_ngay=$so_ngay";
$ch = curl_init(); 
...
Tổng Tiền: '.number_format($tongtien).'đ
    ...
    $update = $NTD->update("users", [
     ...
     $ghilog = $NTD->insert("log_site", [
         'username'=>$username_site_con['username'],
                'note'          => ' Đã Tạo Giao Dịch '.$action.' Với Số Tiền '.$tongtien.', Time '.gettime().'',
                'ip'            => getip(),
                'date'          => gettime(),
                 'url_config'          => $domain_site_me
                ]);
