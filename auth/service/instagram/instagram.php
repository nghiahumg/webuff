...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
 ...
 $ghichu      = check_string($_POST['ghichu']);
    ...
    $api_token = api_token();
    $username_khach = $NTD->get_row(" SELECT * FROM `users` WHERE `token` = '$api_token' ");
    
    if($action=="like"){
        $link_post="api/service/instagram/like-post/order";
}
 if($action=="follow"){
        $link_post="api/service/instagram/sub/order";
}
    if(empty($server_buy)){
        $return['status'] = false;
        $return['error'] = true;
...
// LƯU DỮ LIỆU LỊCH SỬ MUA
        $create = $NTD->insert("history_buy", [
            ...
            'soluong'       => $soluong,
            'tong_tien'     => $tongtien,
            'link_buy'     => $id, 'number_star'=> $number_star,
...
$url_post="https://$mien_site_me_thuong/auth/service/instagram/site_con.php?id=$id&soluong=$soluong&server=$server_buy&code_dich_vu=$code_dich_vu&action=$action&loai_camxuc=$loai_camxuc&time_buy=$time_buy&list_msg=$list_msg&domain_site_con=$mien_site_con&domain_site_me=$mien_site_me";
$ch = curl_init(); 
