...
");
   ...
   if($check_api['status']=="wait"){
    ...
    if($action=="like"){
        $link_post="instagram-like";
         $code_dich_vu="like_ins";
}
 elseif($action=="follow"){
        $link_post="instagram-follow";
         $code_dich_vu="cmt_ins";
}
 elseif($action=="cmt"){
        $link_post="instagram-comment";
         $code_dich_vu="sub_ins";
}
else{
   $return['status'] = false;
        $return['error'] = true;
      ...
      elseif($username_admin['capbac']==99){
        ...
        $ratesv_site_me= $gia_dich_vu_site_me['rate_service']+$gia_dich_vu_site_me['rate_service']*$chiet_khau_auto/100;
         ...
         if($action=="sub-sale"){
          ...
          $url="https://graph.instagram.com/$id/subscribers?summary=true&fields=id&limit=1&access_token=$token_fb";
  $ch = curl_init();
