...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
   ...
   $server_buy   = check_string($_POST['server']); $ghichu   = check_string($_POST['ghichu']);
     //KIỂM TRA DATA
    ...
    if($action=="cmt"){
   $link_post="api/service/tiktok/comment/order";
   
}
elseif($action=="like"){
    $link_post="api/service/tiktok/like/order";
    
}
elseif($action=="follow"){
    $link_post="api/service/tiktok/sub/order";
}
elseif($action=="share"){
    $link_post="api/service/tiktok/share/order";
}
   elseif($action=="view"){
    $link_post="api/service/tiktok/view/order";
}
...
");
  ...
  $get_status_site_con = $NTD->get_row(" SELECT * FROM `setting` WHERE `url_config` = '".$url_site_name."'");
    $username_admin = $NTD->get_row(" SELECT * FROM `users` WHERE `token` = '".$get_status_site_con['token_auto_dvfb']."'  AND  `url_config` = '".$get_site['site_me']."'
");
...
$url_post="https://$mien_site_me_thuong/auth/service/tiktok/site_con.php?id=$id&soluong=$soluong&server=$server_buy&code_dich_vu=$code_dich_vu&action=$action&loai_camxuc=$loai_camxuc&time_buy=$time_buy&list_msg=$list_msg&domain_site_con=$mien_site_con&domain_site_me=$mien_site_me";
$ch = curl_init(); 
...
// LƯU DỮ LIỆU LỊCH SỬ MUA
    ...
    $create_ls_site = $NTD->insert("history_buy", [
            ...
            'type'          => $code_dich_vu,
            'hinh_thuc' => $type_buy, 'ghichu'        => $ghichu,
            'soluong'       => $soluong,
...
// LƯU DỮ LIỆU LỊCH SỬ MUA
        $create = $NTD->insert("history_buy", [
         ...
         $create23 = $NTD->insert(" 'ghichu'        => $ghichu,_buy", [
            'username'      => $get_site['username'],
            'type'          => $code_dich_vu,
            'hinh_thuc' => $type_buy,
            'soluong'       => $soluong,
