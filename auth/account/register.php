    ...
    if (empty(check_string($_POST['name'])) || empty(check_string($_POST['email'])) || empty(check_string($_POST['username'])) || empty(check_string($_POST['password']))) {
        ...
        if (check_string($_POST['_token']) != x_csrf_token()) {
            ...
            if (check_username(check_string($_POST['username'])) == false) {
                $return['status'] = false;
                $return['error'] = true;
                $return['data'] = '<div class="alert text-white bg-danger text-center" role="alert"><strong>Vui lòng nhập định dạng tài khoản hợp lệ!</strong></div>';
                ...
                if (strlen(check_string($_POST['username'])) < 6 || strlen(check_string($_POST['username'])) > 32) {
                    $return['status'] = false;
                    $return['error'] = true;
                    $return['data'] = '<div class="alert text-white bg-danger text-center" role="alert"><strong>Vui lòng nhập tài khoản từ 6 đến 32 ký tự !</strong></div>';
                    ...
                    if ($NTD->get_row(" SELECT * FROM `users` WHERE `username` = '" . check_string($_POST['username']) . "' AND  `url_config` = '" . $url_site_name . "' ")) {
                        $return['status'] = false;
                        $return['error'] = true;
                        $return['data'] = '<div class="alert text-white bg-danger text-center" role="alert"><strong>Tên đăng nhập đã tồn tại trên hệ thống!</strong></div>';
                        ...
                        if (check_email(check_string($_POST['email'])) == false) {
                            $return['status'] = false;
                            $return['error'] = true;
                            $return['data'] = '<div class="alert text-white bg-danger text-center" role="alert"><strong>Vui lòng nhập định dạng email hợp lệ!</strong></div>';
                            $return['message']   = 'Vui lòng nhập định dạng email hợp lệ !';
                            die(json_encode($return));
                        } else {
                            if ($NTD->get_row(" SELECT * FROM `users` WHERE `email` = '" . check_string($_POST['email']) . "' AND  `url_config` = '" . $url_site_name . "' ")) {
                                $return['status'] = false;
                                $return['error'] = true;
                                $return['data'] = '<div class="alert text-white bg-danger text-center" role="alert"><strong>Email đã tồn tại trên hệ thống !</strong></div>';
                                $return['message']   = 'Email đã tồn tại trên hệ thống !';
                                die(json_encode($return));
                            } else {
                                if ($NTD->num_rows(" SELECT * FROM `users` WHERE `ip` = '" . getip() . "' AND  `url_config` = '" . $url_site_name . "'") >= 3) {
                                    $return['status'] = false;
                                    $return['error'] = true;
                                    $return['data'] = '<div class="alert text-white bg-danger text-center" role="alert"><strong>Bạn đã bị giới hạn tạo tài khoản, vui lòng liên hệ admin để khôi phục lại!</strong></div>';
                                    $return['message']   = 'Bạn đã bị giới hạn tạo tài khoản, vui lòng liên hệ admin để khôi phục lại!';
                                    die(json_encode($return));
                                } else {

                                    if ($NTD->get_row(" SELECT * FROM `users` WHERE `cuphap` = '" . $NTD->site('cuphap') . rand(0, 9999) . "' AND  `url_config` = '" . $url_site_name . "'")) {
                                        $cuphapnap = $NTD->site('cuphap') . " " . $_POST['username'];
                                    } else {
                                        $cuphapnap = $NTD->site('cuphap') . " " . $_POST['username'];
                                    }

                                    $create = $NTD->insert("users", [
...
Thời Gian: ' . $timenow . ' 
               ');
                                        $return['status'] = true;
                                        $return['error'] = false;
                                        $return['data'] = '<div class="alert text-white bg-success text-center" role="alert"><strong>Đăng ký thành công, chuyển hướng sau 1s!</strong></div>';
                                        $return['message']   = 'Đăng ký thành công';
                                        die(json_encode($return));
                                    } else {
                                        $return['status'] = false;
                                        $return['error'] = true;
                                        $return['data'] = '<div class="alert text-white bg-danger text-center" role="alert"><strong>Đã xảy ra lỗi khi đăng ký, vui lòng liên hệ admin!</strong></div>';
