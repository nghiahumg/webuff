...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
   ...
   if($_POST['action']=="change-token"){
    ...
    $api_token = api_token();
    $checkusite = $NTD->get_row(" SELECT * FROM `users` WHERE `token` = '$api_token' ");
    
     $token_site = $NTD->get_row(" SELECT * FROM `ds_sitecon` WHERE `token` = '$api_token' ");
    ...
    if(empty($api_token)){
       ...
       if($update2 = $NTD->update("users", ['token' => $token_new], " `username` = '".$checkusite['username']."' AND  `url_config` = '".$url_site_name."' ")){
           
            $check_site = $NTD->get_row(" SELECT * FROM `ds_sitecon` WHERE `token` = '$api_token'  AND  `url_config` = '".$url_site_name."'
");
               ...
               $update_token_new = 
               $NTD->update("ds_sitecon", ['token' => $token_new], "  `username` = '".$my_user."' ");
    ...
    if($update2 = $NTD->update("users", ['password' => typepass($new_password)], " `username` = '".$checkusite['username']."' ")){
        $return['status'] = true;
        $return['error'] = false;
    ...
    if($type == 'collaborators'){
        if($checkusite['tongnap'] < $NTD->site('rate_ctv')){
            $return['status'] = false;
            $return['error'] = true;
            $return['message']   = "Bạn chưa đủ điều kiện để nâng lên cấp bậc cộng tác viên";
            die(json_encode($return));
        }else{
            $update1 = $NTD->update("users", ['capbac' => 1], " `username` = '".$checkusite['username']."' ");
    ...
    if($type == 'agency'){
        if($checkusite['tongnap'] < $NTD->site('rate_daily')){
            $return['status'] = false;
            $return['error'] = true;
            $return['message']   = "Bạn chưa đủ điều kiện để nâng lên cấp bậc đại lý";
            die(json_encode($return));
        }else{
            $update2 = $NTD->update("users", ['capbac' => 2], " `username` = '".$checkusite['username']."' ");
        ...
        if($type == 'agencyy'){
        if($checkusite['tongnap'] < $NTD->site('rate_admin')){
            $return['status'] = false;
            $return['error'] = true;
            $return['message']   = "Bạn chưa đủ điều kiện để nâng lên cấp bậc nhà phân phối";
            die(json_encode($return));
        }else{
            $update1 = $NTD->update("users", ['capbac' => 4], " `username` = '".$checkusite['username']."' ");
