...
// config data
{
    ...
    $username = $NTD->check('account_mbbank');
    $password = $NTD->check('password_mbbank');
}

$conn = mysqli_connect($serverdb, $udb, $pdb, $ndb) or die('Error connection');
$MBBANK = new MBBANK($conn);

$getlistcard = $NTD->get_list("SELECT * FROM `mbbankauto` WHERE 1");
    ...
    foreach ($getlistcard as $rows) {
        ...
        if (!empty($lsgd['result']['transactionHistoryList']) || $lsgd['result']['transactionHistoryList'] != null) {
         ...
         //$description =  $transactionHistoryList[$i]['description'];
                $contentok =  trim(str_replace('.', '', explode('TU:', explode($NTD->site('cuphap'), $content)[1])[0]));
             $description =  trim(str_replace('.', '', explode('TU:', explode($NTD->site('cuphap'), $description)[1])[0]));
                $io =  $transactionHistoryList[$i]['io'];
                if ($io == 1) {

                    $checuNHQ = $NTD->get_row("SELECT * FROM `users` WHERE `id` = '$contentok' AND `url_config` = '" . $rows['sitename'] . "'");

                    if ($checuNHQ) {
                        $checktranid = $NTD->get_row("SELECT * FROM `history_naptien` WHERE `type` = 'Bank' AND `tranid` = '$tranIdok' AND `url_config` = '" . $rows['sitename'] . "'");
                        ...
                        if (!$checktranid) {
                            ...
                            if ($create) {
                                /* update tiền user*/
                                $update1 = $NTD->cong("users", "money", $creditAmount, " `id` = '$contentok' AND `url_config` = '" . $rows['sitename'] . "'");
                                $update3 = $NTD->cong("users", "tongnap", $creditAmount, " `id` = '$contentok' AND `url_config` = '" . $rows['sitename'] . "'");
