...
$getlistcard = $NTD->get_list("SELECT * FROM `cron_momo` WHERE 1");
 ...
 print_r($gethistt);
        if ($gethistt['momoMsgHistory'] != null) {
            foreach ($gethistt['momoMsgHistory'] as $listhis) {
                ...
                $comment = str_replace(' ', '', trim(ltrim($listhis['comment'], $NTD->site('cuphap'))));
                $amount = $listhis['amount'];
                $partnerName = $listhis['partnerName'];
                // echo $comment.'<br>'
                $checuNHQ = $NTD->get_row("SELECT * FROM `users` WHERE `id` = '$comment' AND `url_config` = '" . $rows['sitename'] . "'");

                if ($checuNHQ) {
                    $checktranid = $NTD->get_row("SELECT * FROM `history_naptien` WHERE `type` = 'Bank' AND `tranid` = '$tranId' AND `url_config` = '" . $rows['sitename'] . "'");
                    ...
                    if (!$checktranid) {
                        ...
                        if ($create) {
                            /* update tiền user*/
                            $update1 = $NTD->cong("users", "money", $amount, " `id` = '$comment' AND `url_config` = '" . $rows['sitename'] . "'");
                            $update3 = $NTD->cong("users", "tongnap", $amount, " `id` = '$comment' AND `url_config` = '" . $rows['sitename'] . "'");
