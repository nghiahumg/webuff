...
$getlistcard = $NTD->get_list("SELECT * FROM `thesieureauto` WHERE 1");
    ...
    foreach ($getlistcard as $value) {
        ...
        if (!empty($response)) {
            ...
            foreach ($Get_Table->find('tr') as $Data) {
                ...
                $usernamex = trim(explode('|||', $Data->find('td', 6)->plaintext)[0], $NTD->site('cuphap'));
                ...
                $checktheu = $NTD->get_row("SELECT * FROM `users` WHERE `id` = '$usernamex' AND `url_config` = '" . $value['sitename'] . "'");

                if ($checktheu) {
                    if (substr($Data->find('td', 2)->plaintext, 0, 1) == '+') {
                        $checkthe = $NTD->get_row("SELECT * FROM `history_naptien` WHERE `type` = 'Bank' AND `tranid` = '$tranid' AND `url_config` = '" . $value['sitename'] . "'");
                        ...
                        if (!$checkthe) {
                            ...
                            if ($create) {
                                /* update tiền user*/
                                $update1 = $NTD->cong("users", "money", $PricesExchange, " `id` = '$usernamex' AND `url_config` = '" . $value['sitename'] . "'");
                                $update3 = $NTD->cong("users", "tongnap", $PricesExchange, " `id` = '$usernamex' AND `url_config` = '" . $value['sitename'] . "'");
