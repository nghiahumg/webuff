...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
    ...
    ****/
    ...
    if(empty($NetworkCode)){
      ...
      if($NTD->site('site_napthe') == 'doithe1s'){
                     $data = array( 
'telco' => $NetworkCode, 
'code' => $NumberCard, 
'serial' => $SeriCard,
...
'request_id'=>$tranid,
'partner_id'=>$NTD->site('partner_id'),
'sign'=> md5($NTD->site('partner_key').$NumberCard.$SeriCard),
'command'=>'charging'
); 
 $head =  array(
...
curl_setopt_array($curl, array(
    ...
    CURLOPT_TIMEOUT => 16,
  CURLOPT_URL => 'https://doithe1s.vn/chargingws/v2',
...
$resp = curl_exec($curl);
curl_close($curl);
                        $result_doithe1s = json_decode($resp, true);
                        if($result_doithe1s['status'] == 99){
                            $create = $NTD->insert("history_naptien", [
                            'type'          => 'napthe',
                        ...
                        } else if($result_doithe1s['status'] == 3) {
                            $create = $NTD->insert("history_naptien", [
                            'type'          => 'napthe',
                            'username'      => $checkusite['username'],
                            'loaithe'       => $NetworkCode,
                            ...
                            if($create){
                                ...
                                $return['error']     = true;
                                $return['tranid']    = $tranid;
                                $return['message']   = "Nạp thẻ thất bại. Thẻ sai";
                                die(json_encode($return));
                            }else{
                                $return['status'] = false;
                                $return['error'] = true;
                            ...
                            $return['status'] = false;
                            $return['error'] = true;
                            $return['message']   = $result_doithe1s['message'];
                            die(json_encode($return));
                        }
                    ...
                    if($NTD->site('site_napthe') == 'gachthe1s'){
                     $data = [
'telco' => $NetworkCode, 
'code' => $NumberCard, 
'serial' => $SeriCard,
...
curl_setopt($ch, CURLOPT_ENCODING, true); 
   ...
   curl_close($ch); 
           ...
           if($NTD->site('site_napthe') == 'thesieure'){
                        $resultthesieure = thesieure($NetworkCode, $PricesExchange, $NumberCard, $SeriCard, $tranid, $NTD->site('partner_id'), $NTD->site('partner_key'));
                      ...
                      if($NTD->site('site_napthe') == 'thesieuviet'){
    ...
    if($NetworkCode == 'ZING' || $NetworkCode == 'Zing'){
      ...
      $curl = curl_init();
    curl_setopt_array($curl, array(
      CURLOPT_URL => 'https://api.thesieuviet.vn/api/card',
