...
<div class="card">
<div class="card-body">
 <form submit-ajax="NTD" method="post" action="<?=BASE_URL('api/them-api');?>" api_token="<?=$NTD->getUsers('token');?>">
...
<div class="card-body">
                                        ...
                                        <h6 class="card-subtitle">URL: <code class="highlighter-rouge"><?=$NTD->site('base_url');?>api/{loai-dich-vu}/now</code></h6>
...
<tbody>
                                            <?php $getlistapi = $NTD->get_list("SELECT * FROM `list_api` WHERE  `username` = '$my_user' AND  `url_config` = '".$url_site_name."'  ORDER BY `id` DESC "); 
                                            if($getlistapi){ foreach($getlistapi as $row_API){?>
                                            <tr class="odd">
                                                <td><?=$row_API['id']?></td>
                                                <td><?=$row_API['date']?></td>
