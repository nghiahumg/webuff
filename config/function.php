...
$NTD = new NTD;
$api_momo = $NTD->site('api_automm');
$base_url = 'https://' . $_SERVER['SERVER_NAME'] . '/';
$user_auto_dvfb = $NTD->site('user_auto_dvfb');
$pass_auto_dvfb = $NTD->site('pass_auto_dvfb');
$token_auto_dvfb = $NTD->check('token_auto_dvfb');
$url_site_name = strtoupper($_SERVER['SERVER_NAME']);
$token_auto_site = $NTD->site('token_autodv');
$URL_ADMIN = $NTD->check('url_admin');
$lam_tron = $NTD->site('trang_thai_lam_tron');
$token_fb = $NTD->site('token_facebook');
$themes_landing = $NTD->site('theme_landing');
$themes_login = $NTD->site('theme_login');
$theme_home = $NTD->site('theme_home');




if ($NTD->getUsers('capbac') == 0) {
    $chietkhau = $NTD->site('ck_user');
    $level = 'Thành Viên';
} elseif ($NTD->getUsers('capbac') == 0) {
    $chietkhau = $NTD->site('ck_ctv');
    $level = 'Cộng Tác Viên';
} elseif ($NTD->getUsers('capbac') == 2) {
    $chietkhau = $NTD->site('ck_dl');
    $level = 'Đại Lý';
} elseif ($NTD->getUsers('capbac') == 3) {
    $level = 'Nhà Phân Phối';
    $chietkhau = $NTD->site('ck_npp');
} elseif ($NTD->getUsers('capbac') == "99") {
    $chietkhau = "0";
    $level = 'Quản Trị Viên';
} else {
    #
...
$callback_cardvip = $NTD->site('base_url') . 'api/callback_cardvip.php';
...
$site_gmail = $NTD->site('mail_config');
$site_pass = $NTD->site('pass_mail_config');
function sendCSM($mail_nhan, $ten_nhan, $chu_de, $noi_dung, $bcc)
{
    ...
    $mail->SMTPAuth = true;
    $mail->Username = $NTD->site('mail_config'); // GMAIL STMP
    $mail->Password = $NTD->site('pass_mail_config'); // PASS STMP
    $mail->SMTPSecure = 'tls';
    $mail->Port = 587;
    $mail->setFrom($NTD->site('mail_config'), $bcc);
    $mail->addAddress($mail_nhan, $ten_nhan);
    $mail->addReplyTo($NTD->site('mail_config'), $bcc);
    $mail->isHTML(true);
    $mail->Subject = $chu_de;
    $mail->Body    = $noi_dung;
    $mail->CharSet = 'UTF-8';
...
{
    $url = 'https://doithe1s.vn/chargingws/v2?sign=' . md5($partner_key . $pin . $serial) . '&telco=' . $telCo . '&code=' . $pin . '&serial=' . $serial . '&amount=' . $amount . '&request_id=' . $requestId . '&partner_id=' . $partner_id . '&command=charging';
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $data = curl_exec($ch);
